...
Total tokens: ~1B (1e9, 2e7 per shard for 50 shards)
"""
...
def process_shard(args, vocab_size):
    ...
    with open(shard, "r") as f:
        data = json.load(f)
    all_tokens = []
    for example in (pbar := tqdm(data, position=shard_id)):
        text = example["story"]
        text = text.strip()  # get rid of leading/trailing whitespace
        tokens = enc.encode(
            text, bos=False, eos=True
        )  # encode the text, use EOS as SEP. No BOS.
        all_tokens.extend(tokens)
        pbar.set_postfix({"tokens": len(all_tokens)})
    ...
    print(
        f"Saved {tokenized_filename}, tokens: {all_tokens.size}, average seqlen: {avg_seq_len:.2f}"
    )
...
def pretokenize(vocab_size):
    ...
    with ProcessPoolExecutor() as executor:
        token_counts = list(executor.map(fun, enumerate(shard_filenames)))
